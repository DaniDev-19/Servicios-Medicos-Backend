...
const updateEnfermedad = async (req, res, next) => {
    ...
    try {
        ...
        const result = await pool.query(`UPDATE enfermedades SET nombre = $1, descripcion = $2, categoria_e_id = $3, estado = $4 WHERE id = $5 RETURNING *`,
...
const getCategoria_e = async (req, res, next) => {
    try {
        const result = await pool.query('SELECT id, nombre FROM categoria_e ORDER BY nombre ASC');
        return res.status(200).json(result.rows);
    } catch (error) {
